...
import { JsonRpcProvider, type BaseProvider } from '@ethersproject/providers';
import {
  ChainId,
  Fraction,
  TradeType,
...
import { Protocol, SwapRouter, ZERO, type Trade } from '@uniswap/router-sdk';
    ...
    ISwapToRatio<AlphaRouterConfig, SwapAndAddConfig>
{
  ...
  public async route(
    ...
    if (tradeType === TradeType.EXACT_OUTPUT) {
      ...
      if (portionAmount?.greaterThan(ZERO)) {
    ...
    if (swapConfig?.simulate && methodParameters && methodParameters.calldata) {
      if (!this.simulator) {
  ...
  private async getSwapRouteFromChain(
    ...
    const mixedProtocolAllowed =
      [ChainId.X1, ChainId.X1_TESTNET].includes(this.chainId) &&
  ...
  > {
    ...
    const [usdPool, nativeAndQuoteTokenV3Pool, nativeAndAmountTokenV3Pool] =
      await Promise.all([
        usdPoolPromise,
